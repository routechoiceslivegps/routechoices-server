...
const populateWebAuthnErrorList = (errors) => {
	const btn = u("#webauthn-feature button");
	btn.find("i").remove();
...
const GuardWebAuthn = () => {
	if (!window.PublicKeyCredential) {
		...
		const webauthn_error = document.getElementById("webauthn-browser-support");
		if (webauthn_error) {
			webauthn_error.style.display = "block";
		}

		const webauthn_label = document.getElementById("webauthn-provision-label");
		if (webauthn_label) {
...
document.addEventListener("DOMContentLoaded", (e) => {
	...
	if (typeof PublicKeyCredential === "undefined") {
		const webAuthnFeature = document.getElementById("webauthn-feature");
		if (webAuthnFeature) {
			webAuthnFeature.style.display = "none";
		}
		const webAuthnUndefinedError = document.getElementById(
